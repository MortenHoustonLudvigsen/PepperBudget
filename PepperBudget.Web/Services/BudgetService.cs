namespace PepperBudget.Web.Services;

public class BudgetService(HttpClient http)
{
    public async Task<List<Account>> GetAccountsAsync(bool internalOnly = false)
    {
    ...
    public async Task<List<Transaction>> GetTransactionsAsync(int? accountId = null)
    {
        var url = accountId.HasValue ? $"api/transactions/{accountId}" : "api/transactions";
        return await http.GetFromJsonAsync<List<Transaction>>(url) ?? [];
    }

    public async Task ImportTransactionsAsync(List<Transaction> transactions) =>
        await http.PostAsJsonAsync("api/transactions", transactions);
